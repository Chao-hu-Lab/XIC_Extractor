...
uv run python scripts\validation_harness.py `
  --suite manual-2raw `
  --grid calibration-v1 `
  --run-id local_minimum_calibration_v1_preset2 `
...
```text
C:\Users\user\Desktop\XIC_Extractor\.worktrees\local-minimum-preset-calibration-v1\output\validation_harness\local_minimum_calibration_v1_preset2\manual_2raw\local_minimum_param_sweep_summary.xlsx
```
...
Update only `resolver_peak_duration_max` in the shipped `local_minimum` preset:

```text
resolver_peak_duration_max: 10.0 -> 2.0
```
...
- The duration cap candidates (`1.5`, `2.0`, `3.0` minutes) do not change the
  manual truth metrics versus current settings in this dataset. That means
  shrinking the default max duration has no observed cost on this clean-matrix
  manual truth sweep. The sweep was rerun after changing the canonical preset to
  `2.0`; `local_minimum_current` retained `0` missing manual peaks, `12` large
  area misses, and `0.034381` area median absolute percent error.
- All local-minimum candidates still show the same max RT outlier
  (`2.119168` minutes), so this run does not provide clean evidence for changing
  search-range or scoring behavior.
...
Keep `legacy_savgol` as the default. Keep the other `local_minimum` preset values
unchanged until a candidate improves area agreement without increasing large
misses. Urine and other complex matrices should be used as a later robustness
evaluation after the clean-matrix model is stable.
