...
本輪後續又試了一個比較不巢狀、比較像通用證據的切法：
`standard_shape_clean_reintegration_stable_candidate_family_trace`。白話說，它問的是
「shape 很乾淨（>=0.95）且 reintegration stable 的 family，拿同 family 既有
detected trace 來遮罩重積分，能不能守住 0.1 min / 10% area？」答案是目前
oracle 20/20 pass：104 個 audit-intersection rows / 33 families，從 31 個
families 的 334 個 detected trace candidates 選 20 個代表案例，最大 boundary
error `0.0830019 min`、最大 area relative error `0.0725986`。但我沒有把它做成
新的 writer flag，因為臨時 writer probe 顯示這 104 列在目前 baseline matrix
全部是既有值，`matrix_cells_written=0`、`unchanged_delta_row_count=104`，
expected-diff fail。結論：shape-clean stability 是可重跑的
...
1. 若要進 PR/merge，下一步是處理 remote/PR 層級。
   ...
   - Remote CI 只代表先前 pushed head；本機分支後續又有多個
      productization commits。推 PR 前先跑 `git status -sb` 和 full local gate，
      push 後再重看 PR CI。
   - commit 時只 stage 這輪要提交的 productization/docs 檔案；不要把無關
      workflow-rule 或其他 agent dirty diff 混入。
