...
- CID-NL is no longer only diagnostic for the narrowed adopted bundle: `cid_nl_default_product_activation_v1` is `production_ready`, has `write_authority=TRUE`, and changes the default quant matrix for exactly 95 cells.
...
| CID-NL default product activation v1 | `production_ready` / `product_ready_default_matrix_activated` | 95 adopted CID-NL cells across 20 transitions are now an explicit default matrix/ProductWriter authority scope under `cid_nl_adopt_ready_feature_inclusion_95_cells`. | Any expansion beyond 95 cells needs a new expected-diff/provenance gate. |
...
- Inspected `cid_nl_default_product_activation_summary.json`: `status=pass`, `activation_label=product_ready_default_matrix_activated`, `product_writer_changed=True`, `default_quant_matrix_changed=True`, `expected_diff_count=95`, `written_backfill_count=95`, `unused_expected_diff_count=0`, `raw_or_85raw_ran=False`.
...
- This does not mean all CID-NL candidates are product-ready. Only the adopted 95-cell bundle is default-active.
...
2. Decide whether the next product goal is CID-NL expansion beyond 95 cells or a different registered lane.
3. If expanding CID-NL, require a new bounded expected-diff/provenance gate before changing any default output.
