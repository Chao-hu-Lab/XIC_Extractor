...
the preferred path for broadening. A real no-RAW 85RAW replay over the existing
4613-row consolidated source audit now passes with 439 generated `write_ready`
rows, 72 `detected_flagged`, and 4102 `blocked`; the product writer wrote exactly
the 439 current approved-evidence rows and `backfill_policy_write_ready_rows`
