...
the preferred path for broadening, but it is not a broad 4613-row
`production_ready` claim until additional evidence classes have oracle and
...
|---|---|---|---|---|
| Primary | `backfill_standard_seed_guard_scope_v1` | none; 72-row high-signal, 42-row low-scan, 57-row low-height, 69-row low-height-low-scan, and 220-row low-height reintegration-stable narrow writer ready slices done; generated broad policy path now exists but only replays generated `write_ready`; apex-delta, width-only, and shape-margin probes are candidate only; reintegration-stability audit still leaves the full 299-row pool as `production_candidate` because only the low-height subset has oracle + writer expected-diff approval | maintain existing explicit scoped writer contracts, prefer the generated policy engine for future broadening, and add broader evidence classes only with masked/product-writer oracle evidence plus expected-diff | stop if the next step would silently broaden matrix writes without expected-diff/oracle evidence, if generated policy rows become a manual allowlist, if apex-delta/width-only/shape-margin or all-stability rows are promoted without resolving heldout oracle failures, or if a RAW rerun would not change the broad-scope decision |
...
- New tier: no broad tier promotion. The generated policy path is
  `production_candidate` infrastructure for lower-manual-intervention
  broadening; only rows classified `write_ready` by already approved evidence
  classes may be replayed through the product writer.
- Evidence: `standard_peak_backfill_productization.py` now accepts
...
- Validation: `$env:UV_CACHE_DIR='.uv-cache'; uv run ruff check xic_extractor\diagnostics\standard_peak_backfill_productization.py tools\diagnostics\standard_peak_backfill_productization.py tests\test_standard_peak_backfill_productization.py`
  ...
  (`3759 passed, 1 skipped`); and `$env:UV_CACHE_DIR='.uv-cache'; uv run python scripts\check_diagnostics_index.py`.
...
- Next checkpoint: run a real no-RAW generated-policy replay on existing 85RAW
  artifacts only if it changes the next decision; otherwise add the next
  evidence class to the policy engine with a focused oracle/expected-diff gate.
