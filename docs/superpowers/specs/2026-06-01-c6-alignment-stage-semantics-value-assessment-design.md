...
**Date:** 2026-06-01
**Status:** Phase 5 implementation snapshot v1.7 — C6-B final owner-family disposition
...
| Owner-family construction: `owner_clustering.cluster_sample_local_owners` and ambiguous review-only features | Called by `run_alignment(...)`, diagnostic probe, and owner-clustering tests | Future cross-sample `TraceGroup` / `PeakHypothesis` family spine may absorb complete-link owner families only after parity-backed migration | C6-B final `keep_as_stage`; active structural input today | Keep tests now. Before any future promotion, preserve complete-link grouping, drift-prior edges, no same-sample merges, neutral-loss/product conflict splits, impossible m/z rejection, edge evidence sink, review-only identity-conflict features, and public parity for `alignment_matrix.tsv`, `alignment_cells.tsv`, `alignment_review.tsv`, and `owner_edge_evidence.tsv` when emitted. |
...
| Hard family split gates | `active_policy` | C6-A3 can expose hard-gate challenge observations only when blocked `OwnerEdgeEvidence` is emitted; envelope-only construction gates remain unprojected active policy. Same-sample, neutral-loss, precursor, product, and observed-loss splits are still construction-time policy. |
| Review-only owner records | `successor_owned` only when review-only facts are projected for a review-only feature | C6-A3 adds `CrossSamplePeakGroupReviewFact` as an internal shadow projection for `identity_conflict` and ambiguous-owner review records. This is review evidence only, not matrix-delivery ownership. |
...
C6-B final disposition for `owner_clustering.py`:
`keep_as_stage`.

C6-B does not attempt a constructor/adaptor experiment. A1/A2/A3 prove useful
shadow projections, but complete-link construction, hard split gate construction
policy, and backfill/matrix delivery remain active blockers. Review-only
projection is shadow evidence, not matrix-delivery ownership.

Exit rule:

```text
Do not promote owner_clustering.py to an internal constructor, compatibility
adapter, or retirement candidate until blocking invariants are successor-owned
and public parity is proven for alignment_matrix.tsv, alignment_cells.tsv,
alignment_review.tsv, and owner_edge_evidence.tsv when emitted. Until then,
owner-family construction is an active owner-first stage.
```
...
| Owner family grouping | `xic_extractor.alignment.owner_clustering.cluster_sample_local_owners` | Builds cross-sample owner families and edge evidence for owner-first matrix delivery; conceptually this is cross-sample hypothesis/family construction, but current downstream still consumes `OwnerAlignedFeature`. | owner-family grouping | Existing owner-clustering tests cover complete-link grouping, drift-prior edges, same-sample exclusion, neutral-loss/product/observed-loss conflict splits, impossible m/z rejection, edge evidence sink, and review-only identity-conflict features. `test_owner_family_construction_is_writer_visible` pins owner-family projection into `alignment_matrix.tsv`, `alignment_cells.tsv`, and `alignment_review.tsv`. C6-B successor contract tests pin A1/A2/A3 shadow evidence and the final `keep_as_stage` disposition, including `owner_edge_evidence.tsv` parity when emitted. | Complete-link gates, hard split gate construction policy, and backfill/matrix delivery could be flattened into review-only shadow facts if promoted too early. | `keep_as_stage` / `contract_only` | `candidate_source` and `structural_input` | C6-B final: no C6 cleanup now. Do not promote to constructor, adapter, or retirement until blocking invariants are successor-owned and parity is proven for `alignment_matrix.tsv`, `alignment_cells.tsv`, `alignment_review.tsv`, and `owner_edge_evidence.tsv` when emitted. |
...
- `keep_as_stage`: owner-first production path stages, owner-family
  construction / `owner_clustering.py` after C6-B, claim registry, primary
