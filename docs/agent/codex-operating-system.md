...
- `docs/superpowers/handoffs/current/<branch-slug>-<topic>.md`: branch-scoped
  ...
  prune only the active branch handoff when status changes; do not update a
  handoff whose `Branch:` / `Status:` belongs to another branch. Handoffs do not
  override the control plane, named specs, validation notes, PR body, or git
  state. Keep them under about 200 lines unless explicitly justified. Completed
  phase summaries belong in archive; long logs and scratch analysis belong in
  notes, not the active handoff.
- `docs/superpowers/handoffs/RETENTION.tsv`: handoff cleanup inventory. Every
  file under handoff `current/` or `archive/` needs one row so PR closeout can
  distinguish active stubs, public evidence, Obsidian-transfer candidates, and
  removal candidates that still require exact manifest plus explicit approval.
- `docs/superpowers/pulse-reports/`: compact read-side productization status
  snapshots generated by `xic-productization-pulse`. They summarize evidence
  freshness, blockers, and next actions; they do not promote tiers.
