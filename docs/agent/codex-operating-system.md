...
- `docs/superpowers/handoffs/current/cc-framework-improvements-productization.md`:
  ...
  prune it when status changes; it does not override the control plane, named
  specs, validation notes, or git state.
  Keep it under about 200 lines unless explicitly justified. Completed phase
  summaries belong in archive; long logs and scratch analysis belong in notes,
...
- `UserPromptSubmit`: scans the prompt for XIC high-risk keywords and injects
  ...
  `docs/superpowers/plans/2026-06-15-productization-control-plane.md`.
  Handoff/closeout/compaction prompts remind the agent to rewrite and prune the
  active handoff snapshot instead of appending chronological notes, including
  the 200-line prune target, archive summaries, and notes for long logs.
...
- `PostToolUse`: catches pytest runs that collected zero tests, and points the
  ...
  of the update, it reminds closeout to rewrite/prune the handoff or state why
  it is still current. It also reminds when the active handoff exceeds the
  200-line target.
