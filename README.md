...
`local_minimum` preset 會保留較寬鬆但語意明確的 MZmine-style local-minimum 起點：`resolver_min_search_range_min=0.08`、`resolver_min_relative_height=0.0`、`resolver_min_ratio_top_edge=1.7`、`resolver_peak_duration_min=0.0`、`resolver_peak_duration_max=2.0`。切換 resolver profile 不會覆蓋另一個 profile 已調整的參數，只有按下 preset button 才會套用預設值。
