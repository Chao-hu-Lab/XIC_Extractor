...
`region_first_safe_merge` 是目前 tracked settings 與 validation harness 的保守預設；它是相容性名稱，底層仍以 local-minimum boundary 加上 safe-merge/WIS promotion 為主。`legacy_savgol` 仍是 accepted clean-trace / compatibility profile，也是 `ExtractionConfig` 的程式相容預設；`local_minimum` 則是直接使用 local-minimum boundary 的 opt-in profile。
