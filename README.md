...
XIC Extractor 會從 Thermo Xcalibur `.raw` 檔案批次提取多目標化合物的 MS1 XIC，寫出 apex RT、raw apex intensity、Gaussian-smoothed positive AsLS residual area（預設 Gaussian15）、peak boundary，並做 MS2 neutral-loss confirmation、peak confidence scoring 與 diagnostics 報告。
...
| `ms1_morphology_smoothing_window_points` | MS1 morphology Gaussian smoothing window points，必須為奇數且 >= 3；預設 15，會影響 Gaussian morphology 面積、shape/pattern evidence 與 boundary projection |
| `peak_rel_height` | scipy peak boundary 的 relative height |
| `peak_min_prominence_ratio` | peak prominence 門檻，使用 smoothed apex 的比例 |
| `resolver_mode` | 峰切割 profile：`legacy_savgol`、`local_minimum` 或 `region_first_safe_merge`；tracked settings 預設為 `region_first_safe_merge` |
...
| `rt_prior_library_path` | developer/debug RT prior library CSV path；leave empty for normal use。這不是 first-run feature，需使用先前已驗證結果建立；若來源 peak 錯誤，會 bias selection |
...
`region_first_safe_merge` 是目前 tracked settings、validation harness 與 `ExtractionConfig` 的保守預設；它是相容性名稱，底層仍以 local-minimum boundary 加上 safe-merge/WIS promotion 為主。`legacy_savgol` 仍是 accepted clean-trace / compatibility profile；`local_minimum` 則是直接使用 local-minimum boundary 的 opt-in profile。
`local_minimum` preset 會保留較寬鬆但語意明確的 MZmine-style local-minimum 起點：`resolver_min_search_range_min=0.08`、`resolver_min_relative_height=0.02`、`resolver_min_ratio_top_edge=1.7`、`resolver_peak_duration_min=0.0`、`resolver_peak_duration_max=2.0`。切換 resolver profile 不會覆蓋另一個 profile 已調整的參數，只有按下 preset button 才會套用預設值。
...
| `isotope_label_type` | optional；ISTD isotope 類型，例如 `deuterated` 或 `heavy_non_deuterium`，用於 paired RT evidence |
| `paired_rt_relation` | optional；paired target/ISTD RT 關係，例如 `istd_not_later_than_pair` 或 `learned_delta_only` |
| `sample_applicability` | optional；預設 `all`。設為 `rna_containing` 時，只有 sample name 顯示含 RNA 的列可輸出/計數該 target |
...
| Mode | 狀態 | 用途 |
| --- | --- | --- |
| `region_first_safe_merge` | Tracked settings default | 相容性名稱；使用 local-minimum boundary，並在完整 selection path 加上 safe-merge/WIS promotion |
| `legacy_savgol` | Accepted compatibility | 以 Savitzky-Golay smoothing 和既有 peak boundary 邏輯為主；保留給 clean trace、既有程式呼叫與比較基準 |
| `local_minimum` | Accepted direct mode | 直接使用 local minimum region 切割多峰 XIC；適合方法開發與複雜基質案例評估 |

三個 accepted mode 共用同一份 `settings.csv`，但 GUI 會分開顯示 profile-specific controls。切換 mode 只切換使用哪套 resolver，不會自動覆蓋另一套參數。Alignment validation 可以接受 `region_first_safe_merge` 作為 public CLI value，但 production extraction config 會記錄為 `local_minimum`，避免把相容性名稱誤當成新的 region-first v2 演算法。
...
| `{label}_Area` | 主要定量指標；使用 configured Gaussian-smoothed positive AsLS residual（預設 Gaussian15）在 selected boundary 內積分 |
...
| `--targeted-ms1-shape-identity-auto-limited-default` | 明確執行 headless limited `NL_FAIL/NO_MS2` rescue：先跑 baseline、建立 `targeted_ms1_shape_identity_v0.tsv` support TSV、再跑 final extraction，並用 expected-diff gate 檢查結果；canonical settings 在無 support TSV 時也會走同一個 bounded default；目前只限 `5-hmdC + 5-medC`，且只能寫 `detected_flagged` |
...
Targeted MS1 shape identity auto-limited rescue 是目前 headless normal CLI 的
bounded default：當 `targeted_ms1_shape_identity_activation_policy` 是
`limited_5hmdc_5medc_v1` 且沒有設定 support TSV 時，CLI 會自動跑
baseline/support/final workflow，並且只有 expected-diff gate 通過才發布
`final/output`。若要保留舊式 explicit support TSV 行為，將 policy 設為
`explicit_support_tsv`，或在單次 run 明確傳入 support TSV / activation policy。
...
| 缺 pythonnet / .NET runtime | 重新安裝 packaged app，或在開發環境使用 Python 3.11-3.13 執行 `uv sync --extra dev`；Python 3.14 目前不支援 pythonnet |
